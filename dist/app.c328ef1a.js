...
var renderInput = function renderInput() {
  var accepts = ['.jpg', '.png', '.jpeg', '.gif'];
  files.insertAdjacentHTML('beforeend', "\n    <input type='file' class=\"file__type\" accept=\"".concat(accepts.map(function (acc) {
    return acc;
  }), "\">\n    <div></div>\n  "));
  var modalContent = renderModal();
...
var renderTasks = function renderTasks(_ref) {
  ...
  var classes = completed ? 'tasks__item tasks__item--toggle' : 'tasks__item';
  tasks.insertAdjacentHTML('beforeend', "\n      <ul class='tasks__inner'>\n        <div class=".concat(classes, " id=").concat(id, ">\n          <li class='tasks__info' data-toggle='toggle'>\n            ").concat(name, "\n            <button class='tasks__remove' data-remove='remove'>Remove</button>\n          </li>\n          <div>\n            ").concat(filesData.length ? filesData.map(function (file) {
    return "\n                        <div>\n                          <img width=\"540px\" class='tasks__image' height=\"300px\" src=\"".concat(file, "\">\n                        </div>\n                      ");
  }) : '', "\n          </div>\n        </div>\n        </ul>\n      "));
  return tasks;
};
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return fetch('http://localhost:3001/tasks').then(function (res) {
              return res.json();
            }).then(function (data) {
...
var methods = function methods() {
  return {
    ...
    onFiles: function onFiles(event) {
      var filesEv = Array.from(event.target.files);

      if (Array.isArray(filesEv)) {
        filesEv.forEach(function (file) {
          var reader = new FileReader();

          reader.onload = function (ev) {
            var result = ev.target.result;
            files.insertAdjacentHTML('beforeend', "\n              <img class=\"preview__image\" width=\"200px\" height=\"200px\" src=\"".concat(result, "\">\n            "));
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                ...
                parentNode = event.target.closest('.tasks__item');
                id = Number(parentNode.id);
                tasksData = tasksData.filter(function (task) {
                  return task.id !== id;
                });
                _context3.next = 7;
                return fetch("http://localhost:3001/tasks/".concat(id), {
                  method: 'DELETE',
                  headers: {
                    'Content-Type': 'application/json'
                  }
          ...
          while (1) {
            switch (_context4.prev = _context4.next) {
              case 0:
                ...
                parentNode = event.target.closest('.tasks__item');
                id = Number(parentNode.id);
                compls = tasksData.find(function (task) {
...
var renderMethods = function renderMethods() {
  var input = document.querySelector('.file__type');
  var button = document.querySelector('[data-add]');
  var close = modal.querySelector('.tasks__close');
